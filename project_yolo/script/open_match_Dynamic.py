    ...
    for image in png_files:
        ...
        if image is not None:
            ...
            if key == ord('w'):
                ...
                cv2.imwrite('../saved_Images/photo_front.png', cv_image)
            elif key == ord('q'):
                print("Exiting!!")
...
def detectEmptyTable(msg):
    ...
    for image in png_files:
        ...
        if image is not None:
            ...
            # detect gynastic ball
            if (image == "../Images/top_camera/mesa_livre.png" and min_val >= 180000000.0 and min_val <= 200000000.0):
                cv2.putText(cv_image_copy, 'BOLA GINASIO', ((center_x - ((width // 2) // 2)), (center_y - (height // 2))), cv2.FONT_HERSHEY_SIMPLEX ,  1, (230,216,173), 2, cv2.LINE_AA) 
                cv2.rectangle(cv_image_copy, location, bottom_right, (230,216,173), 5)

            cv2.imshow("Image_top", cv_image_copy)
            cv2.waitKey(1)
